<template>
  <div class="space-y-4">
    <div
      ...
      :class="{
        'border-gray-300 dark:border-gray-600 hover:border-blue-500 dark:hover:border-blue-400': !isDragging,
    ...
    <div v-if="uploadingImages.size > 0 || uploadedImages.length > 0" class="space-y-3">
      <div class="flex justify-between items-center">
        <h4 class="font-semibold text-gray-900 dark:text-white">Imágenes ({{ uploadedImages.length }})</h4>
        <button 
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        ...
        >
          <div class="relative">
            <img :src="img.temporaryDownloadUrl || img.publicUrl" class="w-full aspect-square object-cover rounded-lg shadow-sm" />
            <div v-if="reorderMode" class="absolute inset-0 bg-black/50 rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div v-if="img.isPrimary && !reorderMode" class="absolute bottom-1 left-1 bg-blue-600 text-white text-[10px] px-1.5 py-0.5 rounded">
            Principal
          </div>
        </div>
      </div>
      ...
      <div v-if="reorderMode" class="flex justify-end gap-2 mt-2">
        <fwb-button size="sm" color="alternative" @click="cancelReorder">Cancelar</fwb-button>
        <fwb-button size="sm" gradient="blue" @click="saveReorder">Guardar orden</fwb-button>
      </div>
    </div>
  </div>
</template>
...
const loadImages = async () => {
  try {
    ...
    emit('images-updated', images.map(img => img.temporaryDownloadUrl || img.publicUrl))
  } catch (error) {
...
const uploadFiles = async (files: File[]) => {
  ...
  for (const file of imageFiles) {
    ...
    try {
      ...
      const { uploadUrl, objectKey } = await propertyService.generateImageUploadUrl(props.propertyId, file);
...
const removeImage = async (imageId: string, index: number) => {
  ...
  try {
    ...
    emit('images-updated', uploadedImages.value.map(img => img.temporaryDownloadUrl || img.publicUrl))
  } catch (error) {
...
const handleDropReorder = (e: DragEvent, index: number) => {
...
const saveReorder = async () => {
  try {
    ...
    emit('images-updated', updated.map(img => img.temporaryDownloadUrl || img.publicUrl))
  } catch (error) {

  ...
  <div class="p-6 space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div>
        ...
        <p class="text-gray-500 text-sm">Propiedades que has guardado como favoritas</p>
      </div>
    ...
    <!-- Error State -->
    <div v-else-if="error" class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl p-4 text-red-700 dark:text-red-400">
      {{ error }}
      <button @click="loadFavoritesAndProperties" class="ml-2 underline hover:text-red-800">Reintentar</button>
    </div>

    <!-- Empty State -->
    <div v-else-if="favoriteProperties.length === 0" class="text-center py-20 bg-gray-50 dark:bg-gray-800 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700">
      <svg class="w-16 h-16 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
      </svg>
      <p class="text-gray-500 dark:text-gray-400 text-lg font-medium mb-2">No tienes propiedades favoritas aún</p>
      <p class="text-gray-400 dark:text-gray-500 text-sm mb-4">Explora nuestras propiedades y marca las que más te gusten</p>
      <router-link to="/properties">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <fwb-card v-for="prop in favoriteProperties" :key="prop.id" class="flex flex-col h-full overflow-hidden border-gray-200 dark:border-gray-700 relative group">
        ...
        <div class="absolute top-3 right-3 z-10 opacity-100 group-hover:opacity-0 transition-opacity">
        ...
        <div class="p-5 flex-1 flex flex-col">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mb-3 line-clamp-2">{{ prop.address }}</p>
          
          <div class="flex justify-between items-end mt-auto">
            <div>
              <p class="text-xs text-gray-400 uppercase font-bold tracking-tighter">Precio</p>
              <p class="text-2xl font-black text-blue-600">${{ (prop.price || 0).toLocaleString() }}</p>
            </div>
            <div class="text-right">
              <p class="text-[10px] text-gray-400 uppercase font-bold">Habitaciones</p>
              <p class="text-sm font-semibold dark:text-gray-200">{{ prop.rooms || '—' }}</p>
            </div>
          </div>

          <!-- Actions -->
          <div class="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
            <router-link :to="`/properties/${prop.id}/request-visit`" custom v-slot="{ navigate }">
              <fwb-button size="sm" gradient="blue" @click="navigate" class="w-full">
                Agendar Visita
              </fwb-button>
            </router-link>
...
const favoriteProperties = ref<any[]>([])
...
const handleImageError = (prop: any) => {
  prop.imageUrls = []
}
...
const loadFavoritesAndProperties = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.detail || err.message || 'Error al cargar tus favoritos'
  } finally {
    loading.value = false
  }
}
...
const removeFavorite = async (propertyId: string) => {
  ...
  try {
    ...
    await Swal.fire({
      title: 'Error',
      text: err.response?.data?.detail || 'No se pudo eliminar de favoritos',
      icon: 'error',
      confirmButtonColor: '#dc2626'
    })
  }
...
const viewDetails = (property: any) => {

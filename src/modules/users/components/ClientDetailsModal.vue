  ...
  <fwb-modal v-if="show" @close="$emit('close')" size="4xl">
    <template #header>
      <div class="flex items-center space-x-4">
        ...
        <StatusBadge v-if="client?.status" :status="String(client.status)" />
      </div>
    </template>

    <template #body>
      <div v-if="client" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div class="space-y-4">
          <div
...
<script setup lang="ts">
  ...
  defineProps<{ show: boolean; client: ClientRecord | null }>();
  defineEmits(['close']);

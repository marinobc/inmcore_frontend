  ...
  <div>
    ...
    <PageContainer class="space-y-6 relative">
      ...
      <template v-else>
        ...
        >
          <UserCard v-for="user in users" :key="user.id" :user="user" @details="openDetailsModal">
            <template #actions>
              <ActionIconGroup
                variant="pill"
                :actions="[
                  {
                    ...
                    tooltip: t('agentOwners.editProfile'),
                    color: 'blue',
                  },
                ]"
        ...
        >
          ...
          <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">
            {{ t('agentOwners.emptyTitle') || 'No se encontraron propietarios' }}
          </h3>
          <p class="text-gray-500 dark:text-gray-400 max-w-xs mx-auto text-sm">
            {{
              t('agentOwners.emptyText') ||
              'No hay propietarios asignados que coincidan con tu búsqueda.'
            }}
          </p>
        </div>
      </template>
...
<script setup lang="ts">
  ...
  import { useAgentOwners } from '../composables/useAgentOwners';
